...
$out=Join-Path $PSScriptRoot '교육자료_완성본'
New-Item -ItemType Directory -Path $out -Force | Out-Null
$src=Join-Path $PSScriptRoot '교육자료_PART2/AI_교학상장_INTRO_PART2_1-12.pptx'
$dst=Join-Path $out 'AI_교학상장_완성본_1-21.pptx'
...
function NewSlide($n,$title,$label){
 ...
 Txt ($section+' / '+$label) 48 25 420 20 10 $muted | Out-Null
 ...
 Txt ('{0:00} / 21' -f $n) 830 509 82 20 10 $muted $false 3 | Out-Null
}
...
if($p.Slides.Count -ne 21){throw 'Slide count must be 21'}
for($i=13;$i -le 21;$i++){$p.Slides.Item($i).Export((Join-Path $out ('slide-{0:00}.png' -f $i)),'PNG',1600,900)}
$p.SaveAs((Join-Path $out 'AI_교학상장_완성본_1-21.pdf'),32)
$p.Close();$app.Quit()
Write-Output 'VERIFIED: 21 slides. All original slide content and supporting parts preserved byte-for-byte.'
Write-Output $dst
